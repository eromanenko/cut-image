...
export function sortDetectedCards() {
    ...
    state.detectedCards.sort((cardA, cardB) => {
        let cyA = cardA.reduce((sum, p) => sum + p.y, 0) / cardA.length;
        let cyB = cardB.reduce((sum, p) => sum + p.y, 0) / cardB.length;
        let cxA = cardA.reduce((sum, p) => sum + p.x, 0) / cardA.length;
        let cxB = cardB.reduce((sum, p) => sum + p.x, 0) / cardB.length;

        let minY_A = Math.min(...cardA.map(p => p.y));
        let maxY_A = Math.max(...cardA.map(p => p.y));
        let hA = maxY_A - minY_A;
